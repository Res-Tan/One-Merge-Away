...
# class ModelMergingAV(torch.nn.Module):
#     def __init__(self, model_name, model_list, tokenizer):
#         super(ModelMergingAV, self).__init__()
#         self.model_name = model_name
#         self.tokenizer = tokenizer

#         pt_model = model_list[0]
#         _, names = make_functional(pt_model)
#         self.merged_model = pt_model
#         self.names = names
...
#         self.model_state_dicts = model_state_dicts
#         # 初始化可训练权重参数（通过Softmax约束和为1）
#         # random_param = torch.randn(len(self.model_state_dicts), dtype=torch.float16, requires_grad=True, device="cuda")
#         self.lambda_list = nn.Parameter(torch.zeros(len(self.model_state_dicts), dtype=torch.float16, requires_grad=True, device="cuda")) # 初始化为均匀权重
#         # self.raw_weights.to('cuda')

#     def lambdas(self):
#         lambda_list = F.softmax(self.lambda_list, dim=0)
#         return lambda_list
    
#     def update_merged_model(self):
#         alph = self.lambdas()
#         # alph[0] 是权重向量，paramslist 是多个模型参数组成的列表
#         params = tuple(
#             sum(
#                 pi * alpha_i
#                 for pi, alpha_i in zip(param_group, alph.cpu())
#             )
#             for param_group in zip(*self.model_state_dicts)
#         )
...
#         self.merged_model.to('cuda:0')
#         # print('Merge Finished')
#         print(f'Merge parameter: [{str(alph.data.tolist())}]')
...
# class ModelMergingTA(torch.nn.Module):
#     def __init__(self, model_name, model_list, tokenizer, dataset_probe_benign, dataset_probe_harmful, scale_func, seed):
#         super(ModelMergingTA, self).__init__()
#         self.model_name = model_name
#         self.tokenizer = tokenizer
#         self.scale_func = scale_func
...
#     def get_init_states(self, prompt, init_adv_string):
#         init_adv_tokenids = (
#             self.tokenizer.encode(init_adv_string, add_special_tokens=False, return_tensors="pt")
#             .squeeze(0)
...
#         merged_embed_layer = self.merged_model.get_input_embeddings()
        
#         full_before_adv_embed = merged_embed_layer(full_before_adv_ids)
#         full_after_adv_embed = merged_embed_layer(full_after_adv_ids)
...
#         output = self.merged_model(inputs_embeds=init_full_embed, output_hidden_states=True)
...
#         hidden_states_start_point = hidden_states_start_point.detach()
#         return hidden_states_start_point, adv_tokenids, full_before_adv_ids, full_after_adv_ids
    
#     def compute_grad(self, adv_tokenids, hidden_states_start_point, full_before_adv_ids, full_after_adv_ids, detach_params=True):
#         merged_embed_layer = self.merged_model.get_input_embeddings()
#         vocab_size = merged_embed_layer.weight.shape[0]

#         full_before_adv_embed = merged_embed_layer(full_before_adv_ids)
#         full_after_adv_embed = merged_embed_layer(full_after_adv_ids)

#         adv_onehot = (
#             tokenids2onehot(adv_tokenids, vocab_size, merged_embed_layer.weight.dtype)
#             .unsqueeze(0)
#             .detach()
#             .clone()
#             .to(self.merged_model.device)
#         )
#         adv_onehot.requires_grad_(detach_params)
#         if detach_params:
...
#         adv_embed = adv_onehot @ merged_embed_layer.weight
...
#             dim=1,
#         )
#         outputs = self.merged_model(inputs_embeds=full_embed, output_hidden_states=True)
#         adv_hidden_states = outputs.hidden_states[-1][:, -1, :]

#         vector_from_start_to_here_batch = adv_hidden_states - hidden_states_start_point
...
#         if detach_params:
#             return adv_onehot.grad.squeeze(0), loss, full_before_adv_embed, full_after_adv_embed
#         else:
#             return adv_onehot, loss, full_before_adv_embed, full_after_adv_embed

#     def update_merged_model(self, detach_params=True):
#         alph = self.lambdas()
#         params = tuple(
#             sum(pi * alpha_i for pi, alpha_i in zip(param_group, alph[0].cpu()))
#             for param_group in zip(*self.params_list)
#         )
#         if detach_params:
#             params = tuple(p.detach().cuda(0) for p in params)
#         else:
#             params = tuple(p.cuda(0) for p in params)
#         load_weights(self.merged_model, self.names, params)
#         self.merged_model.to("cuda:0")
...
#     def train_merged_param(self, prompt, adv_string, learning_rate, training_epoch):
#         optimizer = torch.optim.Adam([self.lambda_list], lr=learning_rate, eps=1e-4, betas=(0.9, 0.999), weight_decay=0.)
#         for k in range(training_epoch):
#             optimizer.zero_grad()
            
#             import ipdb
#             ipdb.set_trace()

#             hidden_states_start_point, adv_tokenids, full_before_adv_ids, full_after_adv_ids = self.get_init_states(prompt, adv_string)
#             _, loss, _, _ = self.compute_grad(adv_tokenids, hidden_states_start_point, full_before_adv_ids, full_after_adv_ids)
...
#             M_model.update_merged_model()
#             # print(f'Merge parameter: [{str(M_model.lambdas().data.tolist())}]')
#             merged_loss_list.append((-loss).item())
#             merged_lambda_list.append(str(M_model.lambdas()[0].data.tolist()))
...
#     #         sum(
#     #             pi * alpha_i
#     #             for pi, alpha_i in zip(param_group, alph[0].cpu())
#     #         )
#     #         for param_group in zip(*self.params_list)
#     #     )

#     #     # Break lambda -> merged-weights graph so repeated backward in attack loop is safe.
#     #     params = tuple(p.detach().cuda(0) for p in params)
#     #     load_weights(self.merged_model, self.names, params)
#     #     self.merged_model.to('cuda:0')
#     #     # print('Merge Finished')
#     #     # print(f'Merge parameter: [{str(alph[0].data.tolist())}]')
#     #     self.update_probing_weight()
